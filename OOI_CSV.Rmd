...
Store the csv content in a data frame, select a cast and plot a profile! R is not a fan of verbose column names with spaces and punctuation, so we have to rename before plotting. It also looks like these are duplicate discrete analyses. There's no field for which replicate, so maybe separate by position?
